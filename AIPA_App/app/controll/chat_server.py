...
try:
    from overlay import DesktopGridOverlayManager

    GRID_OVERLAY_IMPORT_ERROR = ""
except Exception as exc:
    DesktopGridOverlayManager = None
    GRID_OVERLAY_IMPORT_ERROR = str(exc)
...
GEMINI_API_KEY = os.getenv(
    "GEMINI_API_KEY", os.getenv("AIPA_GEMINI_API_KEY", "")
).strip()
GEMINI_BASE_URL = os.getenv(
    "AIPA_GEMINI_URL", "https://generativelanguage.googleapis.com/v1beta"
).rstrip("/")
HF_FALLBACK_ENABLED = os.getenv("AIPA_ENABLE_HF_FALLBACK", "0").strip().lower() in {
    "1",
    "true",
    "yes",
}
WEB_SEARCH_ENABLED = os.getenv("AIPA_ENABLE_WEB_SEARCH", "1").strip().lower() in {
    "1",
    "true",
    "yes",
}
...
COMPUTER_CONTROL_ENABLED = os.getenv(
    "AIPA_ENABLE_COMPUTER_CONTROL", "1"
).strip().lower() in {"1", "true", "yes"}
COMPUTER_CONTROL_ROOT = Path(
    os.getenv("AIPA_COMPUTER_CONTROL_ROOT", str(BASE_DIR))
).resolve()
COMPUTER_CONTROL_ALLOW_ANY_PATH = os.getenv(
    "AIPA_COMPUTER_CONTROL_ALLOW_ANY_PATH", "0"
).strip().lower() in {
    "1",
    "true",
    "yes",
}
COMPUTER_CONTROL_ALLOW_DELETE = os.getenv(
    "AIPA_COMPUTER_CONTROL_ALLOW_DELETE", "1"
).strip().lower() in {
    "1",
    "true",
    "yes",
...
COMPUTER_CONTROL_BUILD_TAG = os.getenv(
    "AIPA_COMPUTER_CONTROL_BUILD_TAG", "computer-control-2026-03-12a"
).strip()
...
try:
    KNOWLEDGE_MATCH_THRESHOLD = float(
        os.getenv("AIPA_KNOWLEDGE_MATCH_THRESHOLD", "0.68")
    )
except ValueError:
    KNOWLEDGE_MATCH_THRESHOLD = 0.68

CONTROL_GRID_OVERLAY_MANAGER = (
    DesktopGridOverlayManager(BASE_DIR, rows=CONTROL_GRID_ROWS, cols=CONTROL_GRID_COLS)
    if DesktopGridOverlayManager
    else None
)
if CONTROL_GRID_OVERLAY_MANAGER is not None:
    try:
        CONTROL_GRID_OVERLAY_MANAGER.stop()
    except Exception:
        pass
...
class GridOverlayShowRequest(BaseModel):
    rows: int = Field(default=CONTROL_GRID_ROWS, ge=1, le=12)
    cols: int = Field(default=CONTROL_GRID_COLS, ge=1, le=12)
    focus: str = Field(default="", max_length=16)
...
class ComputerControlSaveRequest(BaseModel):
    prompt: str = Field(min_length=3, max_length=500)
    trigger: Optional[str] = Field(default=None, max_length=250)
...
class KnowledgeStore:
    ...
    def _parse_qa_line(raw_line: str) -> Optional[Tuple[str, str]]:
        ...
        qa_match = re.match(
            r"^(?:q|question|hoi)\s*:\s*(.+?)\s*(?:a|answer|dap)\s*:\s*(.+)$",
            line,
            flags=re.IGNORECASE,
        )
        if qa_match:
    ...
    def _build_seed_signature(files: List[Path]) -> str:
        ...
        for file_path in files:
            try:
                stat = file_path.stat()
                parts.append(
                    f"{file_path.as_posix()}|{stat.st_mtime_ns}|{stat.st_size}"
                )
            except Exception:
    ...
    def _refresh_seed_pairs(self, force: bool = False):
        ...
        if not force and signature == self._seed_signature:
            return
        primary_pairs = (
            self._load_seed_pairs([self.seed_file]) if self.seed_file.exists() else []
        )
        ...
        with self.lock:
            existing = next(
                (
                    item
                    for item in self._pairs
                    if self._normalize(item.get("question", "")) == q
                ),
                None,
            )
            if existing:
    ...
    def _find_answer_in_pairs(
        self, question: str, pairs: List[dict], threshold: float
    ) -> Optional[str]:
        if not pairs:
            return None
        ...
        for item in pairs:
            ...
            keyword_qa = 0.0
            if ask_token_set:
                keyword_q = len(ask_token_set.intersection(saved_q_tokens)) / len(
                    ask_token_set
                )
                keyword_qa = len(ask_token_set.intersection(saved_qa_token_set)) / len(
                    ask_token_set
                )
    ...
    def find_answer(self, question: str) -> Optional[str]:
        ...
        return self._find_answer_in_pairs(
            question, self._combined_pairs(), threshold=KNOWLEDGE_MATCH_THRESHOLD
        )
...
class ConversationStore:
    ...
    def _extract_facts(cls, user_text: str) -> List[str]:
        ...
        if (
            len(clean) <= 80
            and clean.startswith("toi o ")
            and len(clean) > len("toi o ")
        ):
    ...
    def get_recent_messages(
        self, session_id: Optional[str], limit: int = 30
    ) -> List[ChatMessage]:
        ...
        with self.lock:
            ...
            if user_clean:
                session["messages"].append(
                    {"sender": "user", "text": user_clean, "at": now}
                )
            if bot_clean:
                session["messages"].append(
                    {"sender": "bot", "text": bot_clean, "at": now}
                )
...
class WebSearchService:
    def __init__(
        self, enabled: bool, serper_api_key: str, serper_url: str, serpapi_api_key: str
    ):
    ...
    def search_google(self, prompt: str, limit: int = 1) -> List[dict]:
        ...
        results = (
            self._search_with_serpapi(prompt, limit=search_limit)
...
def _should_use_web_search(prompt: str) -> bool:
    ...
    if any(marker in clean for marker in factual_markers):
        return True
    return any(
        keyword in clean for keyword in search_intent_keywords + freshness_keywords
    )
...
def _extract_short_answer(prompt: str, content: str, title: str = "") -> str:
    ...
    if any(
        marker in clean_prompt
        for marker in ("la ai", "la gi", "nuoc nao", "la nuoc nao", "co gi")
    ):
        return _first_sentence(clean_content)
...
def _search_web_answer(prompt: str) -> Optional[str]:
    if (
        ...
        or not web_search_service.active
        or not web_search_service.google_ready
    ):
        return None
    try:
        results = web_search_service.search_google(
            prompt, limit=WEB_SEARCH_SOURCE_LIMIT
        )
    except Exception:
        return None
    if not results:
...
def _merge_histories(
    memory_history: List[ChatMessage], request_history: List[ChatMessage]
) -> List[ChatMessage]:
    ...
    def generate(
        ...
        for model_name in model_candidates:
            ...
            if isinstance(data.get("error"), dict):
                message = (
                    str(data["error"].get("message", "")).strip()
                    or "Gemini trả lời không xác định."
                )
...
_EXE_DISCOVERY_CACHE_LOCK = threading.Lock()
_EXE_DISCOVERY_CACHE: Dict[str, Tuple[float, List[Tuple[int, Path, str]]]] = {}
_REQUIREMENTS_CACHE: Tuple[Optional[int], List[str]] = (None, [])
_REQUIREMENTS_CACHE_LOCK = threading.Lock()
APP_LAUNCHER_CACHE_TTL_SECONDS = max(
    10, int(os.getenv("AIPA_APP_LAUNCHER_CACHE_TTL", "300"))
)
EXE_DISCOVERY_CACHE_TTL_SECONDS = max(
    30, int(os.getenv("AIPA_EXE_DISCOVERY_CACHE_TTL", "600"))
)
EXE_DISCOVERY_SCAN_LIMIT = max(
    1000, int(os.getenv("AIPA_EXE_DISCOVERY_SCAN_LIMIT", "12000"))
)
EXE_DISCOVERY_TIME_BUDGET_SECONDS = max(
    2, int(os.getenv("AIPA_EXE_DISCOVERY_TIME_BUDGET", "8"))
)
MOUSE_MOVE_STEP_PX = max(10, int(os.getenv("AIPA_MOUSE_MOVE_STEP_PX", "90")))
_MOUSE_KEYBOARD_CONTROLLER = None


def _load_keyword_list_from_txt(
    file_path: Path, fallback_keywords: List[str]
) -> List[str]:
    ...
    if not normalized_keywords:
        normalized_keywords = [
            _normalize_match_ascii(item)
            for item in fallback_keywords
            if _normalize_match_ascii(item)
        ]
...
def _invalidate_computer_control_rule_cache():
    global _CONTROL_RULE_CACHE
    _CONTROL_RULE_CACHE = (None, [])


def _build_control_rule_line(trigger_display: str, actions: List[str]) -> str:
    trigger_text = str(trigger_display or "").strip()
    action_text = "||".join(
        [str(action).strip() for action in actions if str(action).strip()]
    )
    if not trigger_text:
        raise ValueError("Trigger thao tác đang trống.")
    if not action_text:
        raise ValueError("Danh sách thao tác đang trống.")
    return f"{trigger_text}=>{action_text}"


def _upsert_computer_control_rule(
    trigger_display: str, actions: List[str]
) -> Dict[str, object]:
    trigger_text = str(trigger_display or "").strip()
    trigger_key = _normalize_match_ascii(trigger_text)
    if not trigger_key:
        raise ValueError("Trigger thao tác không hợp lệ.")

    line_to_save = _build_control_rule_line(trigger_text, actions)
    if COMPUTER_CONTROL_TRAIN_FILE.exists():
        try:
            lines = COMPUTER_CONTROL_TRAIN_FILE.read_text(encoding="utf-8").splitlines()
        except Exception:
    ...
    for raw_line in lines:
        ...
        existing_trigger = stripped.split("=>", 1)[0].strip()
        if _normalize_match_ascii(existing_trigger) == trigger_key:
            updated_lines.append(line_to_save)
            replaced = True
            continue
        updated_lines.append(line)
    ...
    COMPUTER_CONTROL_TRAIN_FILE.parent.mkdir(parents=True, exist_ok=True)
    COMPUTER_CONTROL_TRAIN_FILE.write_text(
        "\n".join(updated_lines).rstrip() + "\n", encoding="utf-8"
    )
    _invalidate_computer_control_rule_cache()
    rules = _load_computer_control_rules()
    return {
        "trigger_display": trigger_text,
        "trigger_key": trigger_key,
        "actions": actions,
        "replaced": replaced,
        "rules_count": len(rules),
        "train_file": COMPUTER_CONTROL_TRAIN_FILE.as_posix(),
        "rule_line": line_to_save,
    }
...
def _ensure_desktop_write_target(path: Path):
    ...
    try:
        relative_target = target.relative_to(CONTROL_DESKTOP_ROOT)
    except ValueError as exc:
    ...
    if lowered_parts.intersection(blocked_system_parts):
        raise PermissionError(
            "Đường dẫn nhạy cảm hệ thống. Từ chối thao tác để bảo vệ máy."
        )

    for part in relative_target.parts:
        clean_part = part.strip()
        if not clean_part:
            raise PermissionError("Tên file hoặc thư mục không hợp lệ.")
        if ":" in clean_part:
            raise PermissionError(
                "Không cho phép alternate data stream hoặc device path."
            )

        stem = clean_part.rstrip(" .").split(".", 1)[0].lower()
        if stem in WINDOWS_RESERVED_DEVICE_NAMES:
            raise PermissionError(
                "Tên file hoặc thư mục trùng thiết bị hệ thống của Windows."
            )
...
def _resolve_desktop_open_path(raw_path: str) -> Path:
    resolved = _resolve_desktop_write_path(raw_path)
    if not resolved.suffix:
        try:
            sibling_matches = [
                candidate
                for candidate in resolved.parent.glob(f"{resolved.name}.*")
        ...
        if len(sibling_matches) == 1:
            sibling_candidate = sibling_matches[0].resolve()
...
def _open_desktop_path(path: Path) -> str:
    if os.name != "nt":
        raise OSError("Hành động mở file hiện chỉ hỗ trợ trên Windows.")
    if not path.exists():
        raise FileNotFoundError("Không tìm thấy tệp hoặc thư mục trong Desktop.")
    os.startfile(str(path))  # type: ignore[attr-defined]
    time.sleep(0.45)
    if path.is_dir():
        return f"Đã mở thư mục: {path.name}"
    return f"Đã mở tệp: {path.name}"
...
def _ensure_deletion_allowed(path: Path):
    if not COMPUTER_CONTROL_ALLOW_DELETE:
        raise PermissionError(
            "Xóa file/thư mục đang bị tắt. Bật AIPA_COMPUTER_CONTROL_ALLOW_DELETE=1 để sử dụng."
        )
...
def _get_system_datetime_summary() -> Dict[str, str]:
    now = datetime.now()
    return {
        "time_hms": now.strftime("%H:%M:%S"),
        "time_hm": now.strftime("%H:%M"),
        "date_dmy": now.strftime("%d/%m/%Y"),
        "date_text_vi": f"ngày {now.strftime('%d')} tháng {now.strftime('%m')} năm {now.strftime('%Y')}",
        "weekday_vi": [
            "Thứ Hai",
            "Thứ Ba",
            "Thứ Tư",
...
def _scan_desktop_clock_text() -> str:
    ...
    for _, class_name, text in prioritized:
        ...
        if token:
            system_info = _get_system_datetime_summary()
            return (
                f"Giờ hiện tại trên máy là {token}, {system_info['date_text_vi']} "
                f"({system_info['weekday_vi']})."
            )

    system_info = _get_system_datetime_summary()
    return (
        f"Giờ hiện tại trên máy là {system_info['time_hms']}, "
        f"{system_info['date_text_vi']} ({system_info['weekday_vi']})."
    )
        ...
        try:
            with winreg.OpenKey(hive, key_path) as app_paths_key:
                ...
                while True:
                    ...
                    executable_path = _extract_executable_path_from_command(
...
def _load_windowsapps_alias_index() -> Dict[str, Path]:
    ...
    alias_dir = (
        Path(os.getenv("LOCALAPPDATA", "").strip()) / "Microsoft" / "WindowsApps"
    )
    index: Dict[str, Path] = {}
    if alias_dir.exists():
        try:
...
def _suggest_windows_app_names(app_name: str, limit: int = 6) -> List[str]:
    ...
    ranked = sorted(
        ...
        key=lambda item: (
            item[0],
            SequenceMatcher(None, _normalize_match_ascii(item[1]), query).ratio(),
        ),
        reverse=True,
    )
...
def _build_app_name_candidates(raw_target: str) -> List[str]:
    ...
    if (
        ...
        and normalized_tokens[0] == "ung"
        and normalized_tokens[1] == "dung"
    ):
...
def _iter_windows_executable_search_roots() -> List[Path]:
    roots: List[Path] = []
...
def _scan_windows_executable_matches(
    app_name: str, limit: int = 6
    ...
    cache_key = f"{query}|{max(1, limit)}"
    now = time.time()
    with _EXE_DISCOVERY_CACHE_LOCK:
        cached = _EXE_DISCOVERY_CACHE.get(cache_key)
        if cached and (now - cached[0]) < EXE_DISCOVERY_CACHE_TTL_SECONDS:
    ...
    deadline = now + EXE_DISCOVERY_TIME_BUDGET_SECONDS
    scanned_files = 0
    scored: Dict[str, Tuple[int, Path, str]] = {}

    for root in _iter_windows_executable_search_roots():
        for current_root, dir_names, file_names in os.walk(root):
            if time.time() > deadline or scanned_files >= EXE_DISCOVERY_SCAN_LIMIT:
                break

            filtered_dirs = []
            for dir_name in dir_names:
                normalized_dir = _normalize_match_ascii(dir_name)
                if normalized_dir in excluded_dirs:
                    continue
            ...
            for file_name in file_names:
                if time.time() > deadline or scanned_files >= EXE_DISCOVERY_SCAN_LIMIT:
                    break
                if not str(file_name).lower().endswith(".exe"):
                    continue
                scanned_files += 1
                exe_path = current_root_path / file_name
                score = max(
                    _score_app_name_match(exe_path.stem, query, tokens),
                    _score_app_name_match(file_name, query, tokens),
                    _score_app_name_match(exe_path.parent.name, query, tokens),
                )
                if score < 18:
                    continue
                path_key = str(exe_path).lower()
                existing = scored.get(path_key)
                if existing is None or score > existing[0]:
                    scored[path_key] = (score, exe_path, "disk-scan")
        if time.time() > deadline or scanned_files >= EXE_DISCOVERY_SCAN_LIMIT:
            break
    ...
    ranked = sorted(
        ...
        key=lambda item: (
            item[0],
            SequenceMatcher(None, _normalize_match_ascii(item[1].stem), query).ratio(),
        ),
        reverse=True,
    )[: max(1, limit)]

    with _EXE_DISCOVERY_CACHE_LOCK:
        _EXE_DISCOVERY_CACHE[cache_key] = (time.time(), list(ranked))
    return ranked


def _discover_windows_app_launchers(
    app_name: str, limit: int = 6
) -> List[Tuple[int, Path, str]]:
    ...
    def add_path(path: Path, source: str, score_override: Optional[int] = None):
        try:
            candidate_path = path.resolve()
        except Exception:
            candidate_path = path
        path_key = str(candidate_path).lower()
        score = score_override
        if score is None:
            query = _normalize_match_ascii(target)
            tokens = [token for token in query.split() if token]
            score = max(
                _score_app_name_match(candidate_path.stem, query, tokens),
                _score_app_name_match(candidate_path.name, query, tokens),
            )
        if score is None or score < 18:
            return
        current = scored.get(path_key)
        if current is None or score > current[0]:
            scored[path_key] = (score, candidate_path, source)
    ...
    for candidate in candidate_names:
        ...
        if found_path:
            add_path(Path(found_path), "path")

    app_paths_executable = _find_windows_app_paths_executable(target)
    if app_paths_executable:
        add_path(app_paths_executable, "app-paths")

    shortcut = _find_windows_start_menu_shortcut(target)
    if shortcut:
        add_path(shortcut, "start-menu")

    windowsapps_alias = _find_windowsapps_alias_executable(target)
    if windowsapps_alias:
        add_path(windowsapps_alias, "windows-apps")

    for score, path, source in _scan_windows_executable_matches(
        target, limit=max(limit, 6)
    ):
        add_path(path, source, score)
    ...
    ranked = sorted(
        ...
        key=lambda item: (
            ...
            SequenceMatcher(
                None,
                _normalize_match_ascii(item[1].stem),
                _normalize_match_ascii(target),
            ).ratio(),
        ),
        reverse=True,
    )
...
def _search_windows_application(app_name: str, limit: int = 5) -> str:
    target = str(app_name or "").strip()
    if not target:
        raise ValueError("Thiếu tên ứng dụng để tìm.")
    if os.name != "nt":
        raise OSError("Tính năng tìm ứng dụng hiện chỉ hỗ trợ trên Windows.")

    matches = _discover_windows_app_launchers(target, limit=max(1, limit))
    if not matches:
        suggestions = _suggest_windows_app_names(target, limit=6)
        if suggestions:
            raise FileNotFoundError(
                "Không tìm thấy file .exe phù hợp. Có thể bạn muốn tìm: "
                + ", ".join(suggestions)
            )
        raise FileNotFoundError(
            "Không tìm thấy file .exe phù hợp cho ứng dụng này trên máy."
        )

    lines = [f'Tìm thấy {len(matches)} ứng dụng phù hợp cho "{target}":']
    for index, (_score, path, source) in enumerate(matches, start=1):
        lines.append(f"{index}. {path.name} [{source}]")
...
def _open_windows_application(app_name: str) -> str:
    ...
    matches = _discover_windows_app_launchers(target, limit=1)
    if matches:
        _score, matched_path, _source = matches[0]
        os.startfile(str(matched_path))  # type: ignore[attr-defined]
        return matched_path.name

    suggestions = _suggest_windows_app_names(target, limit=6)
    if suggestions:
        raise FileNotFoundError(
            "Không tìm thấy ứng dụng cần mở. Có thể bạn muốn mở: "
            + ", ".join(suggestions)
        )
...
def _close_windows_application(app_name: str) -> str:
    ...
    if not scored_processes:
        raise FileNotFoundError(
            f"Không tìm thấy tiến trình đang chạy cho ứng dụng: {target}"
        )
    ...
    matched = [
        (image_name, pid)
        for score, image_name, pid in scored_processes
        if score >= max(8, top_score - 6)
    ]
    if not matched:
...
def _build_control_grid_payload() -> Dict[str, object]:
    screen_width, screen_height = _get_screen_size_for_control()
    cell_width = screen_width / max(1, CONTROL_GRID_COLS)
    cell_height = screen_height / max(1, CONTROL_GRID_ROWS)

    cells: List[Dict[str, object]] = []
    for row_index in range(CONTROL_GRID_ROWS):
        for col_index in range(CONTROL_GRID_COLS):
            label = f"{chr(ord('a') + col_index)}{row_index + 1}"
    ...
    return {
        ...
        "cells": cells,
        "examples": [
            "kéo chuột từ 100,200 đến 300,400",
            "kéo chuột từ a1 đến c3",
            "click 100,200",
            "click a1",
        ],
    }
...
def _get_control_grid_overlay_manager():
    if CONTROL_GRID_OVERLAY_MANAGER is None:
        detail = GRID_OVERLAY_IMPORT_ERROR or "Overlay manager is unavailable."
        raise RuntimeError(f"Overlay tọa độ chưa sẵn sàng: {detail}")
    return CONTROL_GRID_OVERLAY_MANAGER


def _normalize_overlay_focus(value: str) -> str:
    focus = _normalize_match_ascii(value)
    if re.fullmatch(r"[a-z][0-9]+", focus):
        return focus
    return ""


def _extract_control_grid_overlay_focus(prompt: str) -> str:
    normalized = _normalize_match_ascii(prompt)
    matched = re.search(r"\b([a-z][0-9]+)\b", normalized)
    if matched:
        return _normalize_overlay_focus(matched.group(1))
    return ""


def _control_grid_overlay_status() -> Dict[str, object]:
    manager = _get_control_grid_overlay_manager()
    status = manager.get_status()
    return {
        "status": "ok",
        **status,
        **_build_control_grid_payload(),
    }


def _show_control_grid_overlay(
    rows: int = CONTROL_GRID_ROWS, cols: int = CONTROL_GRID_COLS, focus: str = ""
) -> Dict[str, object]:
    manager = _get_control_grid_overlay_manager()
    status = manager.show(
        rows=max(1, int(rows)),
        cols=max(1, int(cols)),
        focus=_normalize_overlay_focus(focus),
    )
    return {
        "status": "ok",
        **status,
...
def _hide_control_grid_overlay() -> Dict[str, object]:
    manager = _get_control_grid_overlay_manager()
    status = manager.hide()
    return {
        "status": "ok",
...
def _is_control_grid_overlay_show_prompt(prompt: str) -> bool:
    normalized = _normalize_match_ascii(prompt)
    if not normalized:
        return False
    if any(token in normalized for token in ("an ", "tat ", "dong ", "hide ")):
        return False
    direct_phrases = (
    ...
    return (
        "luoi" in normalized
        and ("toa do" in normalized or "chuot" in normalized)
        and any(token in normalized for token in ("hien thi", "mo", "bat", "show"))
    )


def _has_control_grid_overlay_keyword(normalized: str) -> bool:
    return any(
...
def _is_control_grid_overlay_hide_prompt(prompt: str) -> bool:
    normalized = _normalize_match_ascii(prompt)
    if not normalized:
        return False
    direct_phrases = (
    ...
    if any(phrase in normalized for phrase in direct_phrases):
        return True
    if _has_control_grid_overlay_keyword(normalized) and any(
        token in normalized for token in ("an", "tat", "dong", "hide")
    ):
        return True
    return (
        "luoi" in normalized
        and ("toa do" in normalized or "chuot" in normalized)
        and any(token in normalized for token in ("an", "tat", "dong", "hide"))
    )


def _handle_control_grid_overlay_prompt(prompt: str) -> str:
    if _is_control_grid_overlay_hide_prompt(prompt):
        _hide_control_grid_overlay()
        return "Đã ẩn lưới tọa độ trên màn hình."
    if _is_control_grid_overlay_show_prompt(prompt):
        focus = _extract_control_grid_overlay_focus(prompt)
        _show_control_grid_overlay(focus=focus)
        target_note = f" tại ô {focus.upper()}" if focus else ""
        return (
            f"Đã hiển thị lưới tọa độ trực tiếp trên màn hình{target_note}. "
            'Bạn có thể nói: "kéo chuột từ 100,200 đến 300,400" hoặc "kéo chuột từ a1 đến c3".'
        )
    return ""
...
def _parse_mouse_target(raw_target: str) -> Tuple[int, int]:
    ...
    if grid_target in {"current", "hien tai", "vi tri hien tai", "tai cho"}:
        controller = _get_mouse_keyboard_controller()
        position = controller.mouse.get_position()
        return int(position[0]), int(position[1])

    # Speech-to-text can transcribe "c4" as "xe 4"/"xem 4".
    grid_target = re.sub(r"^\s*(?:xe|xem)\s+(\d+)\s*$", r"c\1", grid_target)
    grid_target = re.sub(
        r"\b(?:click|nhan|chuot|mouse|tai|vao|o|cell|diem|point|toa do|vi tri)\b",
        " ",
        grid_target,
    )
    grid_target = re.sub(r"\s+", " ", grid_target).strip()

    compact_grid_target = grid_target.replace(" ", "")
    anywhere_grid_match = re.search(r"\b([a-z])\s*([0-9]+)\b", grid_target)
    if anywhere_grid_match:
        compact_grid_target = (
            f"{anywhere_grid_match.group(1)}{anywhere_grid_match.group(2)}"
        )

    if re.match(r"^[a-z][0-9]+$", compact_grid_target):
        try:
        ...
        return resolver.resolve(compact_grid_target)

    # Speech-to-text often outputs "100 phay 200" instead of "100,200".
    numeric_target = _normalize_match_ascii(target)
    numeric_target = re.sub(
        r"\b(?:click|nhan|chuot|mouse|tai|vao|o|cell|diem|point|toa do|vi tri)\b",
        " ",
        numeric_target,
    )
    numeric_target = re.sub(r"\bphay\b", ",", numeric_target)
    numeric_target = re.sub(r"\s+", " ", numeric_target).strip()
    point_match = re.match(r"^\s*(-?\d+)\s*(?:,|\s)\s*(-?\d+)\s*$", target)
    if not point_match:
        point_match = re.match(r"^\s*(-?\d+)\s*(?:,|\s)\s*(-?\d+)\s*$", numeric_target)
    if not point_match:
        raise ValueError(
            'Tọa độ không hợp lệ. Dùng dạng "x,y", "x y" hoặc tọa độ lưới như "c3".'
        )
...
def _split_drag_targets(raw_text: str) -> Tuple[str, str]:
    ...
    normalized_text = _normalize_match_ascii(text)
    grid_matches = re.findall(r"\b([a-z])\s*([0-9]+)\b", normalized_text)
    if len(grid_matches) >= 2:
        return (
            f"{grid_matches[0][0]}{grid_matches[0][1]}",
            f"{grid_matches[1][0]}{grid_matches[1][1]}",
        )

    numeric_text = re.sub(r"\bphay\b", ",", normalized_text)

    point_pair = re.match(
        r"^\s*(?:(?:tu|from)\s+)?(-?\d+\s*(?:,|\s)\s*-?\d+)\s*(?:->|to|den)\s*(-?\d+\s*(?:,|\s)\s*-?\d+)\s*$",
        text,
        flags=re.IGNORECASE,
    )
    if not point_pair:
    ...
    four_numbers_csv = re.match(
        r"^\s*(?:(?:tu|from)\s+)?(-?\d+)\s*,\s*(-?\d+)\s*,\s*(-?\d+)\s*,\s*(-?\d+)\s*$",
        numeric_text,
    )
    if four_numbers_csv:
        return (
    ...
    four_numbers_space = re.match(
        r"^\s*(?:(?:tu|from)\s+)?(-?\d+)\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)\s*$",
        normalized_text,
    )
    ...
    raise ValueError(
        'Lenh keo chuot can 2 diem. Vi du: "a1 b3" hoac "100,200 -> 300,400".'
    )


def _parse_mouse_move_steps(raw_steps: str) -> int:
    value = str(raw_steps or "").strip().replace(",", ".")
    if not value:
        return 1
    try:
        parsed = float(value)
    except ValueError as exc:
        raise ValueError("Số bước di chuột không hợp lệ.") from exc
    if parsed <= 0:
        raise ValueError("Số bước di chuột phải lớn hơn 0.")
...
def _strip_ascii_vowels(text: str) -> str:
    compact = str(text or "").strip()
    return "".join(
        ch for ch in compact if ch.isdigit() or (ch.isalpha() and ch not in "aeiouy")
    )


def _is_fuzzy_control_trigger_match(clean_prompt: str, trigger: str) -> bool:
    prompt_value = str(clean_prompt or "").strip()
    trigger_value = str(trigger or "").strip()
    if not prompt_value or not trigger_value:
        return False
    if prompt_value == trigger_value or prompt_value.startswith(f"{trigger_value} "):
        return True

    compact_prompt = prompt_value.replace(" ", "")
    compact_trigger = trigger_value.replace(" ", "")
    if compact_prompt.startswith(compact_trigger):
        return True

    skeleton_trigger = _strip_ascii_vowels(compact_trigger)
    skeleton_prompt = _strip_ascii_vowels(compact_prompt)
    if len(skeleton_trigger) < 3 or len(skeleton_prompt) < len(skeleton_trigger):
        return False
    return skeleton_prompt.startswith(skeleton_trigger)


def _extract_prompt_tail_by_word_count(prompt: str, trigger_word_count: int) -> str:
    words = str(prompt or "").split()
    if trigger_word_count <= 0 or len(words) <= trigger_word_count:
        return ""
    return " ".join(words[trigger_word_count:]).strip()


def _execute_computer_control_action(
    action_text: str, prompt: str, prompt_tail: str
) -> str:
    ...
    if command in {"SEARCH_APP", "FIND_APP", "LOOKUP_APP", "SEARCH_APPLICATION"}:
        if len(parts) < 2:
            raise ValueError(f"Hành động {command} cần tên ứng dụng.")
        app_target = _render_control_value(parts[1], prompt, prompt_tail)
        return _search_windows_application(app_target)
    ...
    if command in {"MOVE_MOUSE", "MOVE_CURSOR", "MOUSE_MOVE"}:
        if len(parts) < 2:
            raise ValueError(f"Hành động {command} cần hướng di chuyển.")
        direction = _normalize_match_ascii(
            _render_control_value(parts[1], prompt, prompt_tail)
        )
        steps = 1
        if len(parts) >= 3:
            steps = _parse_mouse_move_steps(
                _render_control_value(parts[2], prompt, prompt_tail)
            )

        offset_map = {
            "up": (0, -MOUSE_MOVE_STEP_PX * steps),
            "len": (0, -MOUSE_MOVE_STEP_PX * steps),
            "down": (0, MOUSE_MOVE_STEP_PX * steps),
            "xuong": (0, MOUSE_MOVE_STEP_PX * steps),
            "left": (-MOUSE_MOVE_STEP_PX * steps, 0),
            "trai": (-MOUSE_MOVE_STEP_PX * steps, 0),
            "right": (MOUSE_MOVE_STEP_PX * steps, 0),
            "phai": (MOUSE_MOVE_STEP_PX * steps, 0),
        }
        if direction not in offset_map:
            raise ValueError(
                "Hướng di chuột không hợp lệ. Dùng up/down/left/right hoặc len/xuong/trai/phai."
            )

        controller = _get_mouse_keyboard_controller()
        next_position = controller.mouse.move_relative(offset_map[direction])
        return f"Da di chuot {direction} {steps} buoc den ({int(next_position[0])}, {int(next_position[1])})."
    ...
    if command in {"WAIT", "SLEEP", "DELAY"}:
        ...
        seconds_text = _render_control_value(
            "|".join(parts[1:]), prompt, prompt_tail
        ).strip()
        try:
    ...
    # Desktop scope (safe default): OPEN opens path in Windows; READ previews content in chat.
    if command in {"OPEN_FILE", "OPEN"}:
        raw_path = (
            _render_control_value(parts[1], prompt, prompt_tail)
            if len(parts) >= 2
            else ""
        ...
        file_path = _resolve_desktop_open_path(raw_path)
        return _open_desktop_path(file_path)

    if command in {"READ_FILE", "READ"}:
        raw_path = (
            _render_control_value(parts[1], prompt, prompt_tail)
            if len(parts) >= 2
            else ""
    ...
    if command in {
        ...
        file_path = _resolve_control_path(
            _render_control_value(parts[1], prompt, prompt_tail)
        )
        if not file_path.exists():
            raise FileNotFoundError(
                "Không tìm thấy tệp hoặc thư mục trong vùng control root."
            )
        if file_path.is_dir():
            return _list_dir_preview(file_path)
        return _read_file_preview(file_path)
    ...
    if command in {"APPEND_FILE_REST", "APPENDFILEREST", "APPEND_REST", "APPENDREST"}:
        ...
        if not body_part:
            raise ValueError(
                'Thiếu nội dung cần thêm. Ví dụ: "thêm vào file note.txt: dong 1".'
            )
        file_path = _resolve_desktop_write_path(path_part)
        file_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    if command in {"WRITE_FILE", "WRITE"}:
        ...
        file_path = _resolve_desktop_write_path(
            _render_control_value(parts[1], prompt, prompt_tail)
        )
    ...
    if command in {"CREATE_DIR", "MKDIR", "CREATE_FOLDER"}:
        if len(parts) < 2:
            raise ValueError(f"Hành động {command} cần 1 tham số đường dẫn.")
        dir_path = _resolve_desktop_write_path(
            _render_control_value(parts[1], prompt, prompt_tail)
        )
    ...
    if command in {"DELETE_PATH", "DELETE", "REMOVE"}:
        if len(parts) < 2:
            raise ValueError(f"Hành động {command} cần 1 tham số đường dẫn.")
        path = _resolve_desktop_write_path(
            _render_control_value(parts[1], prompt, prompt_tail)
        )
...
def _render_computer_control_help_text() -> str:
    ...
    quick_examples = [
        "máy tính: mở chrome",
        "máy tính: tìm ứng dụng steam",
        "máy tính: đóng chrome",
        "máy tính: hiển thị lưới tọa độ",
        "máy tính: ẩn lưới tọa độ",
        "máy tính: click a1",
        "máy tính: click 1200,700",
        "máy tính: kéo chuột từ a1 đến c3",
        "máy tính: kéo chuột từ 100,200 đến 300,400",
        "máy tính: cuộn xuống",
    ...
    lines.append("")
    lines.append(
        'Ví dụ nhanh (có thể thêm tiền tố "máy tính:" / "pc:" / "lệnh:" để rõ nghĩa):'
    )
    lines.extend([f"- {item}" for item in quick_examples])
    lines.append(
        '- Lưu ý: lệnh "gõ chữ ..." sẽ dán vào cửa sổ đang có con trỏ nhập liệu của Windows bằng clipboard + Ctrl+V.'
    )
    lines.append("")
    if triggers:
    ...
    lines.append(
        "API: GET /api/computer-control/help, GET /api/computer-control/rules, GET /api/computer-control/overlay/status, POST /api/computer-control/overlay/show, POST /api/computer-control/overlay/hide"
    )
    return "\n".join(lines).strip()


def _extract_saved_control_body(prompt: str) -> str:
    raw_prompt = str(prompt or "").strip()
    if not raw_prompt:
        return ""
    pattern = (
        r"^\s*"
        r"(?:thêm|them|lưu|luu|cấu\s*hình|cau\s*hinh)\s+"
        r"(?:thao\s*tác|thao\s*tac|lệnh|lenh|câu\s*lệnh|cau\s*lenh)\s+"
        r"(.+?)\s*$"
    )
    matched = re.match(pattern, raw_prompt, flags=re.IGNORECASE)
    if matched:
        return str(matched.group(1) or "").strip()
    return raw_prompt


def _split_saved_control_segments(command_text: str) -> List[str]:
    raw = str(command_text or "").strip()
    if not raw:
        return []
    parts = re.split(
        r"\s+(?:và|va|rồi|roi|sau\s+đó|sau\s+do|tiếp\s+theo|tiep\s+theo)\s+",
        raw,
        flags=re.IGNORECASE,
    )
...
def _parse_saved_control_actions(command_text: str) -> List[str]:
    command_body = _extract_saved_control_body(command_text)
    segments = _split_saved_control_segments(command_body)
    if not segments:
        raise ValueError("Không tìm thấy thao tác nào để lưu.")

    actions: List[str] = []
    for segment in segments:
        normalized = _normalize_match_ascii(segment)
        normalized = re.sub(r"\s+", " ", normalized).strip()
        if not normalized:
            continue

        move_match = re.match(
            r"^(?:di|dua)\s+chuot\s+(len|xuong|trai|phai|up|down|left|right)(?:\s+(\d+(?:[.,]\d+)?))?(?:\s+buoc|\s+lan)?$",
            normalized,
        )
        if move_match:
            direction = move_match.group(1)
            steps = _parse_mouse_move_steps(move_match.group(2) or "1")
            actions.append(f"MOVE_MOUSE|{direction}|{steps}")
            continue

        scroll_match = re.match(
            r"^cuon\s+(len|xuong|up|down)(?:\s+(\d+(?:[.,]\d+)?))?(?:\s+buoc|\s+lan)?$",
            normalized,
        )
        if scroll_match:
            direction = scroll_match.group(1)
            amount = _parse_mouse_move_steps(scroll_match.group(2) or "1")
            actions.append(f"SCROLL|{direction}|{amount}")
            continue

        wait_match = re.match(
            r"^(?:cho|doi)\s+(\d+(?:[.,]\d+)?)\s*(?:giay|s)$", normalized
        )
        if wait_match:
            actions.append(f"WAIT|{wait_match.group(1).replace(',', '.')}")
            continue

        if normalized in {"click", "click trai", "nhan chuot", "nhan chuot trai"}:
            actions.append("CLICK|CURRENT")
            continue

        if normalized in {"click phai", "nhan chuot phai"}:
            actions.append("RIGHT_CLICK|CURRENT")
            continue

        type_match = re.match(
            r"^\s*(?:gõ|go|nhập|nhap)(?:\s+(?:chữ|chu))?\s+(.+?)\s*$",
            segment,
            flags=re.IGNORECASE,
        )
        if type_match:
            text_to_type = str(type_match.group(1) or "").strip()
            if not text_to_type:
                raise ValueError("Thiếu nội dung cho thao tác gõ chữ.")
            actions.append(f"TYPE_TEXT|{text_to_type}")
            continue

        hotkey_match = re.match(
            r"^\s*(?:(?:nhấn|nhan)\s+(?:phím|phim)|(?:phím|phim)\s+(?:tắt|tat))\s+(.+?)\s*$",
            segment,
            flags=re.IGNORECASE,
        )
        if hotkey_match:
            key_text = str(hotkey_match.group(1) or "").strip()
            if not key_text:
                raise ValueError("Thiếu tổ hợp phím cho thao tác lưu.")
            actions.append(f"PRESS_KEYS|{key_text}")
            continue

        open_match = re.match(
            r"^\s*(?:mở|mo)(?:\s+(?:ứng\s+dụng|ung\s+dung|app))?\s+(.+?)\s*$",
            segment,
            flags=re.IGNORECASE,
        )
        if open_match:
            app_name = str(open_match.group(1) or "").strip()
            if not app_name:
                raise ValueError("Thiếu tên ứng dụng để lưu thao tác mở app.")
            actions.append(f"OPEN_APP|{app_name}")
            continue

        close_match = re.match(
            r"^\s*(?:đóng|dong|tắt|tat)(?:\s+(?:ứng\s+dụng|ung\s+dung|app))?\s+(.+?)\s*$",
            segment,
            flags=re.IGNORECASE,
        )
        if close_match:
            app_name = str(close_match.group(1) or "").strip()
            if not app_name:
                raise ValueError("Thiếu tên ứng dụng để lưu thao tác đóng app.")
            actions.append(f"CLOSE_APP|{app_name}")
            continue

        raise ValueError(
            f'Chưa hỗ trợ phân tích thao tác: "{segment}". '
            "Hiện hỗ trợ di chuột, cuộn, click, click phải, gõ chữ, nhấn phím, chờ, mở app, đóng app."
        )

    if not actions:
        raise ValueError("Không trích xuất được thao tác nào từ câu lệnh cần lưu.")
    return actions


def _save_computer_control_prompt_rule(
    prompt: str, trigger_override: Optional[str] = None
) -> Dict[str, object]:
    command_body = _extract_saved_control_body(prompt)
    trigger_display = str(trigger_override or "").strip() or command_body
    if not trigger_display:
        raise ValueError("Không tìm thấy câu lệnh kích hoạt để lưu.")

    actions = _parse_saved_control_actions(command_body)
    saved = _upsert_computer_control_rule(trigger_display, actions)
    saved["parsed_prompt"] = command_body
    saved["message"] = f'Đã lưu thao tác "{trigger_display}" thành công.'
    return saved
...
def _try_execute_computer_control(prompt: str) -> Optional[str]:
    ...
    for rule in rules:
        ...
        if not trigger:
            continue
        if _is_fuzzy_control_trigger_match(clean_prompt, trigger):
            prompt_tail_candidate = _extract_prompt_tail(prompt_for_control, trigger)
            if not prompt_tail_candidate:
                prompt_tail_candidate = _extract_prompt_tail_by_word_count(
                    prompt_for_control, len(trigger.split())
                )
            ...
            if not prompt_tail_candidate:
                ...
                if requires_tail:
                    continue
            matched_rule = rule
            matched_prompt_tail = prompt_tail_candidate
            break

    if not matched_rule:
        return None

    trigger_display = str(matched_rule.get("trigger_display", "")).strip() or str(
        matched_rule.get("trigger", "")
    )
    prompt_tail = matched_prompt_tail
    action_logs = []
    for idx, action in enumerate(matched_rule.get("actions", []), start=1):
        try:
            result = _execute_computer_control_action(
                str(action), prompt_for_control, prompt_tail
            )
...
def _word_chain_candidates_for_start(start_key: str, used_keys: set) -> List[str]:
    candidates = [
        phrase
        for phrase in WORD_CHAIN_LEXICON
        if _word_chain_first_key(phrase) == start_key
        and _word_chain_phrase_key(phrase) not in used_keys
    ]
    if not candidates:
        return []
    ...
    def continuation_score(phrase: str) -> int:
        ...
        return sum(
            ...
            if _word_chain_first_key(item) == next_start
            and _word_chain_phrase_key(item) not in used_keys
        )

    return sorted(
        candidates, key=lambda item: (continuation_score(item), len(item)), reverse=True
    )


def _build_word_chain_fallback_phrase(
    required_display: str, used_keys: set
) -> Optional[str]:
    endings = [
        "học",
        "viên",
...
def _handle_word_chain_prompt(
    prompt: str, session_id: Optional[str], store: ConversationStore
) -> Optional[str]:
    ...
    return f'Hop le. Minh noi: "{bot_phrase}". Luot ban, bat dau bang "{next_display}".'


def _rule_based_answer(prompt: str) -> Optional[str]:
    clean = _normalize_match(prompt)
    prompt_kind = _detect_system_datetime_prompt_kind(prompt)
    clean_ascii = _normalize_match_ascii(prompt)
    if not clean_ascii and not prompt_kind:
        return None
    ...
    if prompt_kind == "date":
        return (
            f"Hôm nay là {system_info['date_text_vi']} ({system_info['weekday_vi']})."
        )

    if prompt_kind == "time":
        return (
            f"Bây giờ là {system_info['time_hm']}, "
            f"{system_info['date_text_vi']} ({system_info['weekday_vi']})."
        )
    ...
    if "trung quoc" in clean:
        if any(
            marker in clean
            for marker in ("khai sinh", "tu bao gio", "thanh lap", "ra doi", "lap nuoc")
        ):
            return (
    ...
    is_china_marker = any(
        marker in clean for marker in ("trung quoc", "trung hoa", "china")
    )
    if (
        "noi chien trung quoc" in clean
        or "noi chien trung hoa" in clean
        or (
            is_china_marker
            and (
                "quoc dan dang" in clean
                or "dang cong san" in clean
                or "noi chien" in clean
            )
        )
    ):
...
def _matches_fuzzy_compact_prompt(
    compact_prompt: str, templates: Tuple[str, ...], threshold: float = 0.72
) -> bool:
    ...
    for template in templates:
        if not template:
            continue
        if template in compact_prompt or compact_prompt in template:
            return True
        if SequenceMatcher(None, compact_prompt, template).ratio() >= threshold:
            return True
    return False


def _detect_system_datetime_prompt_kind(prompt: str) -> Optional[str]:
    clean_ascii = _normalize_match_ascii(prompt)
    if not clean_ascii:
        return None
    ...
    date_markers = (
        ...
        "hom nay ngay gi",
        "hom nay thu may",
        "hom nay thu bao nhieu",
        "ngay hom nay bao nhieu",
        "ngay hom nay la gi",
    )
    time_markers = (
        "bay gio may gio",
        "bay gio la may gio",
        "bay gio la bao nhieu gio",
        "may gio roi",
        "gio hien tai",
        "thoi gian hien tai",
        "gio may",
    ...
    if any(marker in clean_ascii for marker in date_markers):
        return "date"
    if any(marker in clean_ascii for marker in time_markers):
        return "time"

    compact_prompt = clean_ascii.replace(" ", "")
    if _matches_fuzzy_compact_prompt(
        compact_prompt,
        (
            "homnaylangaybaonhieu",
            "homnayngaybaonhieu",
            "homnaylangaygi",
            "homnaythumay",
            "ngayhomnaybaonhieu",
...
def _is_system_datetime_prompt(prompt: str) -> bool:
    return _detect_system_datetime_prompt_kind(prompt) is not None
...
def _contextual_rule_answer(prompt: str, history: List[ChatMessage]) -> Optional[str]:
    ...
    recent_user_text = " ".join(
        _normalize_match(item.text) for item in history[-14:] if item.sender == "user"
    )
    is_china_marker = any(
        marker in recent_user_text for marker in ("trung quoc", "trung hoa", "china")
    )
    is_china_civil_war_context = is_china_marker and (
        "noi chien" in recent_user_text
        or "quoc dan dang" in recent_user_text
        or "dang cong san" in recent_user_text
    )

    if not is_china_civil_war_context:
        return None

    if "ai lanh dao" in clean_prompt or clean_prompt in {
        "lanh dao la ai",
        "ai la nguoi lanh dao",
    }:
        return (
            "Trong Nội chiến Trung Quốc, phe Cộng sản do Mao Trạch Đông lãnh đạo, "
    ...
    if (
        "cuoc chien do" in clean_prompt and "dien ra" in clean_prompt
    ) or "dien ra nhu the nao" in clean_prompt:
        return (
...
def _repair_mojibake_text(text: str) -> str:
    ...
    for legacy_codec in ("cp1252", "latin-1", "cp1258"):
        try:
            repaired = raw.encode(legacy_codec, errors="strict").decode(
                "utf-8", errors="strict"
            )
        except Exception:
            continue
    ...
    def score(value: str) -> tuple:
        ...
        marks = (
            sum(1 for ch in nfd if unicodedata.category(ch) == "Mn")
            + value.count("đ")
            + value.count("Đ")
        )
...
def _sanitize_historical_answer(
    prompt: str, history: List[ChatMessage], answer: str
) -> str:
    clean_prompt = _normalize_match(prompt)
    ...
    is_china_marker = any(
        marker in context for marker in ("trung quoc", "trung hoa", "china")
    )
    is_china_civil_war_context = is_china_marker and (
        "noi chien" in context
        or "quoc dan dang" in context
        or "dang cong san" in context
    )
...
web_search_service = WebSearchService(
    WEB_SEARCH_ENABLED, SERPER_API_KEY, SERPER_URL, SERPAPI_API_KEY
)
...
def health_check():
    return {
        ...
        "computer_control_train_mtime_ns": (
            COMPUTER_CONTROL_TRAIN_FILE.stat().st_mtime_ns
            if COMPUTER_CONTROL_TRAIN_FILE.exists()
            else None
        ),
...
@app.get("/api/computer-control/grid")
def computer_control_grid():
    return {
        "status": "ok",
        **_build_control_grid_payload(),
    }
...
@app.get("/api/computer-control/overlay/status")
def computer_control_overlay_status():
    try:
        return _control_grid_overlay_status()
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc


@app.post("/api/computer-control/overlay/show")
def computer_control_overlay_show(request: GridOverlayShowRequest):
    try:
        return _show_control_grid_overlay(
            rows=request.rows, cols=request.cols, focus=request.focus
        )
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc


@app.post("/api/computer-control/overlay/hide")
def computer_control_overlay_hide():
    try:
        return _hide_control_grid_overlay()
    except RuntimeError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc


@app.post("/api/computer-control/save")
def save_computer_control_rule(request: ComputerControlSaveRequest):
    try:
        saved = _save_computer_control_prompt_rule(
            _repair_mojibake_text(request.prompt),
            _repair_mojibake_text(request.trigger or ""),
        )
    except ValueError as exc:
...
def chat(request: ChatRequest):
    ...
    prompt_text = _repair_mojibake_text(request.prompt)
    prompt_for_model = prompt_text
    web_notice = ""
    force_google_lookup = _should_force_google_lookup(prompt_text)
    should_lookup_web = WEB_SEARCH_ENABLED and (
        force_google_lookup
        or WEB_SEARCH_MODE == "always"
        or (WEB_SEARCH_MODE != "off" and _should_use_web_search(prompt_text))
    )

    if _is_system_datetime_prompt(prompt_text):
        rule_answer = _rule_based_answer(prompt_text)
        if rule_answer:
            rule_answer = _finalize_answer_for_response(rule_answer)
            conversation_store.append_exchange(session_id, prompt_text, rule_answer)
            return ChatResponse(
                answer=rule_answer, source="fallback", model="system_datetime"
            )

    control_grid_overlay_answer = _handle_control_grid_overlay_prompt(prompt_text)
    if control_grid_overlay_answer:
        control_grid_overlay_answer = _finalize_answer_for_response(
            control_grid_overlay_answer, max_len=900
        )
        conversation_store.append_exchange(
            session_id, prompt_text, control_grid_overlay_answer
        )
        return ChatResponse(
            answer=control_grid_overlay_answer,
            source="fallback",
            model="computer_control_overlay",
        )

    learned_answer = knowledge_store.find_answer(prompt_text)
    if learned_answer:
        learned_answer = _finalize_answer_for_response(learned_answer)
        conversation_store.append_exchange(session_id, prompt_text, learned_answer)
        return ChatResponse(
            answer=learned_answer, source="knowledge", model="knowledge_store"
        )

    requirements_answer = _answer_from_requirements(prompt_text)
    if requirements_answer:
        requirements_answer = _finalize_answer_for_response(
            requirements_answer, max_len=1200
        )
        if requirements_answer:
            conversation_store.append_exchange(
                session_id, prompt_text, requirements_answer
            )
            return ChatResponse(
                answer=requirements_answer,
                source="knowledge",
                model="requirements_chat",
            )

    if _is_computer_control_help_prompt(prompt_text):
        help_text = _render_computer_control_help_text()
        help_text = _finalize_answer_for_response(help_text, max_len=2600)
        conversation_store.append_exchange(session_id, prompt_text, help_text)
        return ChatResponse(
            answer=help_text, source="fallback", model="computer_control_help"
        )

    computer_control_answer = _try_execute_computer_control(prompt_text)
    if computer_control_answer:
        computer_control_answer = _finalize_answer_for_response(
            computer_control_answer, max_len=1800
        )
        conversation_store.append_exchange(
            session_id, prompt_text, computer_control_answer
        )
        return ChatResponse(
            answer=computer_control_answer,
            source="fallback",
            model="computer_control_train",
        )

    word_chain_answer = _handle_word_chain_prompt(
        prompt_text, session_id, conversation_store
    )
    if word_chain_answer:
        word_chain_answer = _finalize_answer_for_response(word_chain_answer)
        conversation_store.append_exchange(session_id, prompt_text, word_chain_answer)
        return ChatResponse(
            answer=word_chain_answer, source="fallback", model="word_chain"
        )
    ...
    contextual_answer = _contextual_rule_answer(prompt_text, merged_history)
    if contextual_answer:
        contextual_answer = _finalize_answer_for_response(contextual_answer)
        conversation_store.append_exchange(session_id, prompt_text, contextual_answer)
        return ChatResponse(
            answer=contextual_answer, source="fallback", model="contextual_rule"
        )

    prompt_for_model = _apply_language_instruction(prompt_for_model, prompt_text)
    ...
    if ollama_chat_model.enabled:
        ...
        raw_answer = ""
        try:
            raw_answer = ollama_chat_model.generate(
                prompt_for_model, merged_history, memory_facts
            )
        except Exception as exc:
            raw_answer = ""
        generated_answer = _postprocess_answer(raw_answer)
        generated_answer = _sanitize_historical_answer(
            prompt_text, merged_history, generated_answer
        )
        ...
        if generated_answer:
            ...
            conversation_store.append_exchange(
                session_id, prompt_text, generated_answer
            )
            return ChatResponse(
                answer=generated_answer, source="model", model=OLLAMA_MODEL_NAME
            )

    # If local LLM fails (common: not enough RAM), proactively use web search when available.
    if web_allowed and (
        should_lookup_web or (ollama_attempted and not generated_answer)
    ):
        web_answer = _search_web_answer(prompt_text)
        if web_answer:
            answer = _finalize_answer_for_response(web_answer, max_len=1800)
            conversation_store.append_exchange(session_id, prompt_text, answer)
    ...
    if HF_FALLBACK_ENABLED:
        raw_answer = ""
        try:
            raw_answer = text_model.generate(
                prompt_for_model, merged_history, memory_facts
            ).strip()
        except Exception as exc:
            raw_answer = ""
        ...
        if generated_answer:
            ...
            return ChatResponse(
                answer=generated_answer, source="model", model=MODEL_NAME
            )
    ...
    conversation_store.append_exchange(session_id, prompt_text, fallback_answer)
...
def extract_face_embedding(request: FaceExtractRequest):
    ...
    if not embedding:
        raise HTTPException(
            status_code=422, detail="Không tìm thấy khuôn mặt trong ảnh."
        )
