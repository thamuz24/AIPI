...
rem Prefer the nearest ai_service that actually contains ollama.exe (repo checkout vs packaged layout).
set "REPO_AI_SERVICE_DIR=%SCRIPT_DIR%..\ai_service"
if not exist "%REPO_AI_SERVICE_DIR%\ollama\ollama.exe" set "REPO_AI_SERVICE_DIR=%SCRIPT_DIR%..\..\..\ai_service"
if exist "%REPO_AI_SERVICE_DIR%\ollama\ollama.exe" set "AI_SERVICE_DIR=%REPO_AI_SERVICE_DIR%"
