...
def _scan_desktop_clock_text() -> str:
    ...
    return (
        f"Không đọc được text đồng hồ từ desktop. Giờ hệ thống hiện tại: {now_local}."
    )
...
def _try_execute_computer_control(prompt: str) -> Optional[str]:
    ...
    prompt_tail = _extract_prompt_tail(
        prompt_for_control, str(matched_rule.get("trigger", ""))
    )
    action_logs = []
...
def chat(request: ChatRequest):
    ...
    if requirements_answer:
        ...
        if requirements_answer:
            conversation_store.append_exchange(
                session_id, request.prompt, requirements_answer
            )
            return ChatResponse(
    ...
    if computer_control_answer:
        ...
        conversation_store.append_exchange(
            session_id, request.prompt, computer_control_answer
        )
        return ChatResponse(
    ...
    word_chain_answer = _handle_word_chain_prompt(
        request.prompt, session_id, conversation_store
    )
    if word_chain_answer:
        word_chain_answer = _finalize_answer_for_response(word_chain_answer)
        conversation_store.append_exchange(
            session_id, request.prompt, word_chain_answer
        )
        return ChatResponse(
    ...
    if contextual_answer:
        ...
        conversation_store.append_exchange(
            session_id, request.prompt, contextual_answer
        )
        return ChatResponse(
    ...
    if ollama_chat_model.enabled:
        ...
        generated_answer = _sanitize_historical_answer(
            request.prompt, merged_history, generated_answer
        )
        ...
        if generated_answer:
            ...
            conversation_store.append_exchange(
                session_id, request.prompt, generated_answer
            )
            return ChatResponse(
